...
app.get('/health', util.prepareLayout, function(req, res) {
	util.redirect(req, res, "http://www.reddit.com/r/gonewild+nsfw+RealGirls+ass+BustyPetite+girlsinyogapants+dirtysmall+celebnsfw+AsianHotties+asstastic+60fpsporn+datgap+WtSSTaDaMiT+AmateurArchives+IndianBabes+assinthong+buttplug+nsfwCelebArchive+latinas+Exxxtras+KateeOwen+Anjelica_Ebbi+tight__Ozawa+CarolSeleme+wacrover+girlsfinishingthejob", true);
});
...
app.get(/\.(gif|jpg|jpeg|png|css|js)$/, function(req, res) {
	res.statusCode = 404;
